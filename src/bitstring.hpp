...
class BitString
{
  ...
  public: // functions
     ...
     * @pre   i_len <= MAXBITS
     * @pre   i_pos + i_len <= getBitLen()
     * @note  If i_len is greater than MAXBITS the input string is repeated.
     */
    ...
    void setAll() { for ( uint32_t i = 0; i < getBitLen(); i++ ) { setBit(i); } }
    ...
    void clearAll() { for ( uint32_t i = 0; i < getBitLen(); i++ ) { clearBit(i); } }
                           ...
                           uint32_t i_sLen, uint32_t i_dPos )
{
    ...
    packed_buffer<T> * sRelAddr = i_sStr.getRelativePosition( sRelOffset, i_sPos );
    packed_buffer<T> * dRelAddr =        getRelativePosition( dRelOffset, i_dPos );
...
BitStringBuffer<T> BitString<T>::operator>>( uint32_t i_shift ) const
{
    ...
    if ( i_shift < getBitLen() )
    {
        ...
        BitString<T> bso ( bsb.getBitLen() - i_shift, bsb.getBufAddr(), i_shift );
...
BitStringBuffer<T> BitString<T>::operator<<( uint32_t i_shift ) const
{
    ...
    if ( i_shift < getBitLen() )
    {
        ...
        BitString<T> bso ( this->getBitLen() - i_shift, this->getBufAddr(),
                        i_shift );
