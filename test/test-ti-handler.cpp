...
std::string uint_to_str(T data)
{
    ...
    switch (size)
    {
        ...
        case 2:
            return fmt::format("{:04x}", be16toh(data));
        case 4:
            return fmt::format("{:08x}", be32toh(data));
        default:
...
TEST(TiHandler, TestSet0)
{
    ...
    try
    {
        ...
        EXPECT_EQ("0123", uint_to_str<uint16_t>(*(uint16_t*)(&data.data4[0])));
        EXPECT_EQ("01234567",
                  uint_to_str<uint32_t>(*(uint32_t*)(&data.data4[0])));
    }
    catch (const std::invalid_argument& ia)
    {
        trace::err("Invalid argument: %s.", ia.what());
        FAIL();
    }
}
...
TEST(TiHandler, TestSet1)
{
    ...
    try
    {
        ...
        EXPECT_EQ(tiMap["0x0b Num. Hex Words"],
                  uint_to_str<uint8_t>(tiInfo[0x0b]));
        EXPECT_EQ(tiMap["0x0e Length of SRC"],
                  uint_to_str<uint16_t>(*(uint16_t*)(&tiInfo[0x0e])));
        EXPECT_EQ(tiMap["0x10 SRC Word 12"],
                  uint_to_str<uint32_t>(*(uint32_t*)(&tiInfo[0x10])));
        EXPECT_EQ(tiMap["0x14 SRC Word 13"],
                  uint_to_str<uint32_t>(*(uint32_t*)(&tiInfo[0x14])));
        EXPECT_EQ(tiMap["0x18 SRC Word 14"],
                  uint_to_str<uint32_t>(*(uint32_t*)(&tiInfo[0x18])));
        EXPECT_EQ(tiMap["0x1c SRC Word 15"],
                  uint_to_str<uint32_t>(*(uint32_t*)(&tiInfo[0x1c])));
        EXPECT_EQ(tiMap["0x20 SRC Word 16"],
                  uint_to_str<uint32_t>(*(uint32_t*)(&tiInfo[0x20])));
        EXPECT_EQ(tiMap["0x24 SRC Word 17"],
                  uint_to_str<uint32_t>(*(uint32_t*)(&tiInfo[0x24])));
        EXPECT_EQ(tiMap["0x28 SRC Word 18"],
                  uint_to_str<uint32_t>(*(uint32_t*)(&tiInfo[0x28])));
        EXPECT_EQ(tiMap["0x2c SRC Word 19"],
                  uint_to_str<uint32_t>(*(uint32_t*)(&tiInfo[0x2c])));
        EXPECT_EQ(tiMap["0x30 ASCII Data"],
                  uint_to_str<uint32_t>(*(uint32_t*)(&tiInfo[0x30])));
        EXPECT_EQ(tiMap["0x34 ASCII Data"],
                  uint_to_str<uint32_t>(*(uint32_t*)(&tiInfo[0x34])));
        EXPECT_EQ(tiMap["0x38 ASCII Data"],
                  uint_to_str<uint32_t>(*(uint32_t*)(&tiInfo[0x38])));
        EXPECT_EQ(tiMap["0x3c ASCII Data"],
                  uint_to_str<uint32_t>(*(uint32_t*)(&tiInfo[0x3c])));
        EXPECT_EQ(tiMap["0x40 ASCII Data"],
                  uint_to_str<uint32_t>(*(uint32_t*)(&tiInfo[0x40])));
        EXPECT_EQ(tiMap["0x44 ASCII Data"],
                  uint_to_str<uint32_t>(*(uint32_t*)(&tiInfo[0x44])));
        EXPECT_EQ(tiMap["0x48 ASCII Data"],
                  uint_to_str<uint32_t>(*(uint32_t*)(&tiInfo[0x48])));
        EXPECT_EQ(tiMap["0x4c ASCII Data"],
                  uint_to_str<uint32_t>(*(uint32_t*)(&tiInfo[0x4c])));
        EXPECT_EQ(tiMap["0x50 Location"], uint_to_str<uint8_t>(tiInfo[0x50]));
        EXPECT_EQ(tiMap["0x51 Code Sections"],
                  uint_to_str<uint8_t>(tiInfo[0x51]));
...
TEST(TiHandler, TestSet2)
{
    ...
    try
    {
        ...
        EXPECT_EQ(tiMap["0x05 HB_Term. Type"],
                  uint_to_str<uint8_t>(tiInfo[0x05]));
        EXPECT_EQ(tiMap["0x0c HB Dump Flag"],
                  uint_to_str<uint8_t>(tiInfo[0x0c]));
