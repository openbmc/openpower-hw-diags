...
{
    uint32_t chipId            = 0; // ID for proc0.
    std::string expProcLocCode = "Ufcs-P0-C15";
    std::string expPhysDevPath = "physical:sys-0/node-0/proc-0";
    int32_t expPhys_len        = expPhysDevPath.size();
    char* fdtFileName          = "pdbg_dts_test.dtb";
    int fdtFd;
    ...
    pdbg_for_each_class_target("proc", procTrgt)
    {
        ...
        ASSERT_EQ(util::pdbg::getLocationCode(procTrgt), expLocCode);
