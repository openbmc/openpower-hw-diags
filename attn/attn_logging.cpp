               ...
               const std::vector<util::FFDCFile>& i_ffdc)
{
    ...
    switch (i_event)
    {
        ...
        case EventType::AttentionFail:
            ...
            eventValid = true;
            break;
        case EventType::SbeTimeout:
            eventName  = "org.open_power.Processor.Error.SbeChipOpTimeout";
            eventValid = true;
            break;
        default:
            eventValid = false;
...
{
    trace<level::ERROR>("SBE timeout while getting TI info");
    ...
    return event(EventType::SbeTimeout, additionalData,
                 createFFDCFiles(nullptr, 0));
}
