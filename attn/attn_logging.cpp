                     ...
                     std::map<std::string, std::string> i_additional)
{
    ...
    {
        subsystem = std::stoi(i_additional["Subsystem"]);
        tiPel->setSubsystem(subsystem);
    }
    else
    {
        // TODO: Need to verify this about default value.
        trace::err("Error the key %s does not exist in i_additonal",
                   "Subsystem");
        subsystem = std::stoi(i_additional["Subsystem"]);
        tiPel->setSubsystem(subsystem);
    }
    ...
    if (static_cast<uint8_t>(pel::SubsystemID::hypervisor) == subsystem)
    {
        ...
        if (it != i_additional.end())
        {
            srcString = i_additional["SrcAscii"];
        }
        else
        {
            // TODO: Need to verify this in detail.
            trace::err("Error the key %s does not exist in i_additonal",
                       "SrcAscii");
            srcString = i_additional["SrcAscii"];
        }
                    ...
                    char* i_tiInfoData)
{
    ...
    if (nullptr != i_tiInfoData)
    {
        ...
        {
            subsystem = std::stoi(i_additionalData["Subsystem"]);
        }
        else
        {
            // TODO: Need to verify this about default value.
