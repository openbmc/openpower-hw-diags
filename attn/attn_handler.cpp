...
void attnHandler(bool i_breakpoints)
{
                ...
                else
                {
                    ...
                    if (isr_val & isr_mask & 0x00000002)
                    {
                        active_attentions.emplace_back(
                            AttentionType::Vital, VitalPriorityDefault,
                            handleVital, target, i_breakpoints);
                    }
                    ...
                    {
                        active_attentions.emplace_back(
                            AttentionType::Checkstop, CheckstopPriorityDefault,
                    ...
                    {
                        active_attentions.emplace_back(
                            AttentionType::Special, SpecialPriorityDefault,
    ...
    // comparator function needed for heap creation
    ...
    // convert to heap, this only ensures that highest priority is at front
    ...
    // attempt to handle active attentions, stop when one has been handled or
    // all have been attempted
    while (active_attentions.front().isEnabled())
    {
        // handle highest priority attention
        int status = active_attentions.front().handle();

        // move attention to back of vector
        std::pop_heap(active_attentions.begin(), active_attentions.end(),
                      priorityLessThan());

        // disable attention that we are attempting to handle
        active_attentions.back().disable();

        // if attention successfully handled, remove from vector, done
        // handling attentions, unhandled attentions remain in vector
        if (0 == status)
        {
