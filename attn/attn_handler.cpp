 ...
 * @return nothing
 */
void addHbStatusRegs( uint32_t i_procIndex, pdbg_target* i_pibTarget, pdbg_target* i_fsiTarget );
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            // it is, but eBMC may not so we will dump all procs.
...
void addHbStatusRegs( uint32_t i_procIndex, pdbg_target* i_pibTarget, pdbg_target* i_fsiTarget )
{
   ...
   // We only need this for PRIMARY proc, but will just do on all procs for now.
   // All attentions handled are fatal (except for plain breakpoints). 
   // Not going to sweat the plain breakpoint case since this is lab only
   // using cronus.
   ...
   // (Format should resemble FSP: HostBoot Reg:0000283C  Data:AA801504 00000000  Proc:00050001 )
   std::stringstream ss1, ss2, ss3;
       
   ss1 << "HostBoot Reg:" << std::setw(8)
       << std::setfill('0') << std::hex << l_cfamAddr 
       << " Data:" << l_cfamData
       << " Proc:" << std::setw(8) << i_procIndex;

   ss2 << "HostBoot Reg:" << std::setw(8)
       << std::setfill('0') << std::hex << l_scomAddr1
       << " Data:" << std::setw(16) << l_scomData1 
       << " Proc:" << std::setw(8) << i_procIndex;

   ss3 << "HostBoot Reg:" << std::setw(8)
       << std::setfill('0') << std::hex << l_scomAddr2
       << " Data:" << std::setw(16) << l_scomData2 
       << " Proc:" << std::setw(8) << i_procIndex;
