...
bool tiInfoValid(uint8_t* tiInfo)
{
    ...
    if ((nullptr != tiInfo) && (0 != tiInfo[0]))
    {
        ...
        trace::inf("TI data command = 0x02x", (int)tiDataArea->command);
        ...
        if (0xa1 == tiDataArea->command)
        {
            ...
            trace::inf("TI data SRC format = 0x02x",
                       (int)tiDataArea->srcFormat);

            trace::inf("TI data source = 0x02x", (int)tiDataArea->source);
        }
    }
