 ...
 * handle the highest priority attention first. Each attention
 * object on the heap can be marked for removal or kept on the heap
 * if it was not able to be handled.
 *
 * After an attention was handled (or not) the attnHandler logic can
 * decide whether to continue handling other attentions on the heap
 * or exit.
 *
 * Before exiting the attnHandler the logic can choose whether or not
 * to have the gpio monitor coninue to respond to attention gpio
 * events.
 *
...
bool attnHandler()
{
    ...
    bool handleAnother = true; // handle another attentions
