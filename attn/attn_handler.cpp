...
int attnHandler()
{
    ...
    int rc = 1; // return code, assume attention not handled
    ...
    pdbg_for_each_class_target("fsi", target)
    {
        ...
        if (0 != cfamReadProc(target, 0x1007, cfam_isr)) break;
        if (0 != cfamReadProc(target, 0x100d, cfam_isr_mask)) break;
        ...
        if ((1 == rc) && (cfam_isr_true & 0x00000002))
        {
            rc = handleVital(target);
            if (0 == rc) break; // attention handled
        }

        // bit 1 = checkstop (bit 0 is on the "left")
        if ((1 == rc) && (cfam_isr_true & 0x40000000))
        {
            rc = handleCheckstop(target);
            if (0 == rc) break; // attention handled
        }

        // bit 2 = special attention (bit 0 is on the "left")
        if ((1 == rc) && (cfam_isr_true & 0x20000000))
        {
            rc = handleSpattn(target);
            if (0 == rc)  break; // attention handled
        }

        // bit 3 = recoverable error (bit 0 is on the "left")
        if ((1 == rc) && ( cfam_isr_true & 0x10000000 ))
        {
            rc =  handleRecoverable(target);
            if (0 == rc) break; // attention handled
        }
    }

    (0 == rc) ? printf("Attention handled successfully\n")
              : printf("Error! Attention NOT handled\n");

    return rc; // rc == 0, attention handled
}
