        ...
        [this](const boost::system::error_code& ec) {
            if (ec)
            {
                std::string logMessage =
                    "ATTN GPIO Async error: " + std::string(ec.message());
                log<level::INFO>(logMessage.c_str());
            }
            else
            {
...
void AttnMonitor::handleGPIOEvent()
{
    ...
    if (gpiod_line_event_read_fd(iv_gpioEventDescriptor.native_handle(),
                                 &gpioEvent) < 0)
    {
        logMessage = "ATTN GPIO Failed can't read file descriptor!";
        log<level::INFO>(logMessage.c_str());
    }
    else
    {
        switch (gpiod_line_get_value(iv_gpioLine))
        {
            ...
            case 1:
                logMessage = "ATTN GPIO sync!";
                log<level::INFO>(logMessage.c_str());
                break;

            // unexpected value
            default:
                logMessage = "ATTN GPIO read unexpected valuel!";
                log<level::INFO>(logMessage.c_str());
        }
    }
