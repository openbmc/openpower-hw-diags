                    ...
                    TiDataArea* i_buffer)
{
    ...
    ss << "raw:";
    while (tiDataArea <= (uint32_t*)(i_buffer + sizeof(TiDataArea)))
    {
                         ...
                         TiDataArea* i_tiDataArea)
{
    ...
    ss << "0x00 TI Area Valid:" << (uint8_t)i_tiDataArea->tiAreaValid << ":";
    ss << "0x01 Command:" << (uint8_t)i_tiDataArea->command << ":";
    ss << "0x02 Num. Data Bytes:" << be16toh(i_tiDataArea->numDataBytes) << ":";
    ss << "0x04 Reserved:" << (uint8_t)i_tiDataArea->reserved1 << ":";
    ss << "0x06 HWDump Type:" << be16toh(i_tiDataArea->hardwareDumpType) << ":";
    ss << "0x08 SRC Format:" << (uint8_t)i_tiDataArea->srcFormat << ":";
    ss << "0x09 SRC Flags:" << (uint8_t)i_tiDataArea->srcFlags << ":";
    ss << "0x0a Num. ASCII Words:" << (uint8_t)i_tiDataArea->numAsciiWords
       << ":";
    ss << "0x0b Num. Hex Words:" << (uint8_t)i_tiDataArea->numHexWords << ":";
    ...
    ss << "0x50 Location:" << (uint8_t)i_tiDataArea->location << ":";
    ss << "0x51 Code Sections:" << (uint8_t)i_tiDataArea->codeSection << ":";
    ss << "0x52 Additional Size:" << (uint8_t)i_tiDataArea->additionalSize
       << ":";
    ss << "0x53 Additional Data:" << (uint8_t)i_tiDataArea->andData;
                   ...
                   TiDataArea* i_tiDataArea)
{
    ...
    ss << "0x00 TI Area Valid:" << (uint8_t)i_tiDataArea->tiAreaValid << ":";
    ss << "0x04 Reserved:" << (uint8_t)i_tiDataArea->reserved1 << ":";
    ss << "0x05 HB_Term. Type:" << (uint8_t)i_tiDataArea->hbTerminateType
       << ":";
    ss << "0x0c HB Dump Flag:" << (uint8_t)i_tiDataArea->hbDumpFlag << ":";
    ss << "0x0d Source:" << (uint8_t)i_tiDataArea->source << ":";
