                       ...
                       libhei::Signature& o_rootCause)
{
    ...
    auto itr = std::find_if(i_list.begin(), i_list.end(), [&](const auto& t) {
        return (util::hash<libhei::NodeId_t>("TP_LOCAL_FIR") == t.getId() &&
                     ...
                     libhei::Signature& o_rootCause)
{
    // TODO: Consider returning all of them instead of one as root cause.
    auto itr = std::find_if(i_list.begin(), i_list.end(), [&](const auto& t) {
        return (util::hash<libhei::NodeId_t>("PLL_UNLOCK") == t.getId() &&
                                ...
                                libhei::Signature& o_rootCause)
{
    ...
    func __hash = util::hash<libhei::NodeId_t>;
                         ...
                         libhei::Signature& o_rootCause)
{
    ...
    static const auto pb_ext_fir = util::hash<libhei::NodeId_t>("PB_EXT_FIR");
