                                ...
                                const RasDataParser& i_rasData)
{
    for (const auto s : i_list)
    {
        ...
        {
            // Special Cases:
            // If the channel fail was specifically a firmware initiated channel
            // fail (SRQFIR[25]) check for any IUE bits that are on that would
            // have caused that (RDFFIR[17,37]).
