                               ...
                               const std::string& i_action)
{
    ...
    for (const auto& a : i_data.at("actions").at(i_action))
    {
        ...
        else if ("callout_self" == type)
        {
            ...
            // TODO
            trace::inf("callout_self: priority=%s guard=%c",
                       priority.c_str(), guard ? 'T' : 'F');
        }
        ...
        else if ("callout_bus" == type)
        {
            ...
            // TODO
            trace::inf("callout_bus: name=%s guard=%c",
                       name.c_str(), guard ? 'T' : 'F');
        }
        ...
        {
            trace::err("Unsupported action type: %s", type.c_str());
            assert(false);
        }
    }
